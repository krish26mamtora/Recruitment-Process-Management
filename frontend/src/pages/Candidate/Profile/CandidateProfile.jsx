...
const CandidateProfile = () => {
  ...
  const loadProfile = async (uidParam) => {
    ...
    try {
      const res = await fetch(
        import.meta.env.VITE_API_BASE_URL + `/user-profiles/${uid}`,
      );
      if (res.status === 404) {
  ...
  const saveProfile = async () => {
    ...
    try {
      const res = await fetch(
        import.meta.env.VITE_API_BASE_URL + "/user-profiles",
        {
          method: "POST",
  ...
  const handleResumeUpload = async () => {
    ...
    try {
      const res = await fetch(
        import.meta.env.VITE_API_BASE_URL + `/user-profiles/${uidStr}/resume`,
        {
          method: "POST",
          body: formData,
        },
